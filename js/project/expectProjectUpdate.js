                ...
                if (data.result) {
                    ...
                    if (data.data.length > 0) {
                        ...
                        data.data.forEach(function (value, index) {
                            ...
                            temp += '<div class="projectName" style="margin-bottom: 5px;"><a href="/crm/customer/customerrecord.html?id=' + value.id + '">'+value.name+'</a></div>';
                            temp +='<div class="projectName" ><select class="longselect" name="ycjyy" id="'+value.id+'ycjyy" onchange="showOther(\''+value.id+'\');"><option value="'+value.id+'_0">拒绝</option>';
                            temp +='<option value="'+value.id+'_1" select="1" >已中标,暂未下发通知书</option><option value="'+value.id+'_2" select="2">甲方在走签约流程</option><option value="'+value.id+'_4" select="4">进入招投标</option><option value="'+value.id+'_5" select="5">二次合作或多次合作项目</option><option value="'+value.id+'_6" select="6">甲方合作意愿明朗、指定</option><option value="'+value.id+'_7" select="7">项目推进时间节点、落地时间明朗</option><option value="'+value.id+'_8" select="8">重点跟进项目，团队拿下项目意识强烈</option><option value="'+value.id+'_3" select="3">其他</option></select></div><div class="projectName"><input style="background-color: buttonface;display:none;" class="longselect" id="'+value.id+'qt" value="'+value.ycjqtyy+'" /></div>';
                            if("已中标"==value.ycjyy){
							...
							temp +='</select></div>';
							//<option value="53周">53周</option><option value="53周">53周</option><option value="53周">53周</option><option value="53周">53周</option><option value="53周">53周</option>
							...
							if(value.dqjd=='0-项目线索'){
								temp +='<div class="projectName"><select class="longselect" id="'+value.id+'dz"><option disabled="true" value='+1+' select="b1">1.客户关系介入</option><option disabled="true" value='+2+' select="b2">2.甲方内部情况摸查、陌拜</option><option value='+3+' select="b3">3.甲方设计主管（TB）首拜</option><option value='+4+' select="b4">4.甲方决策人（EB）首拜</option><option select="b5">5.邀请甲方考察</option><option select="b6">6.技术交流会</option><option select="b7">7.落实项目信息/图纸</option></select></div>';								
							}else if(value.dqjd=='1-项目立项'){
								temp +='<div class="projectName"><select class="longselect" id="'+value.id+'dz"><option select="b8">8.项目技术交流会</option><option select="b9">9.整体方案专属建议书</option><option select="b10">10.设计咨询合同签订</option><option select="b11">11.PC深化设计合同签订</option><option select="b12">12.报价/项目招投标</option></select></div>';
							}else if(value.dqjd=='2-项目推进'){
								temp +='<div class="projectName"><select class="longselect" id="'+value.id+'dz"><option select="b13">13.PC采购合同商务洽谈</option></select></div>';
							}
